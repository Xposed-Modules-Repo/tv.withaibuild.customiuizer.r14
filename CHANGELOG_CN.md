...
versionCode 216。面向 HyperOS 1 / Android 14。

相较 r14.21.5：

- 修复移动信号和 Wi-Fi 图标移至状态栏左侧不生效的问题，并改善左侧图标的大小与垂直对齐。
- 新增左侧图标大小（75%–125%）和垂直偏移（−6～6 dp）调节，修改后重启系统界面生效。
- 修复进入“系统 → 控制中心”设置页时，页面过渡过程中内容向上偏移的问题。
- 扩充设置搜索，补齐“隐藏图标”等独立页面中的开关项；支持组合页面路径和关键词检索、Wi-Fi/WIFI 匹配，点击结果可定位对应选项。
- 去掉开关项整行重复的按压背景色，保留开关反馈和搜索高亮。
- 关于页维护者更新为 thetvplus。
