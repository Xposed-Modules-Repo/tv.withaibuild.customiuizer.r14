...
`r14.18.6` 完成设置页面按需加载与热路径优化，修复状态栏实际高度、Observer 生命周期、启动激活状态和备份应用列表等问题，并整理杂项与输入法命名。灵动额头隐藏模式已通过实机验证；匹配高度模式的圆角尺寸仍待适配。详细变化见 [CHANGELOG_CN.md](CHANGELOG_CN.md)。
...
本版本已通过完整离线门禁、正式 Release/R8 构建、版本、v2 签名、zipalign、`debuggable=false` 和 Xposed 元数据校验。固定状态栏高度修改后需完整重启；灵动额头隐藏模式与重启后的高度匹配已完成实机验证，匹配高度模式的圆角尺寸尚未同步。
